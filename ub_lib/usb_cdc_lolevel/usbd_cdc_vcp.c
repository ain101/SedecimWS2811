...
{
/*
  usb_packet_t *packet = (usb_packet_t *)((uint8_t *)(Buf) - 8);
  if (packet->len > 0) {
	  packet->index = 0;
	  packet->next = NULL;
	  if(APP_tx_ptr_head == NULL){
		  APP_tx_ptr_head = packet;
	  } else {
		  APP_tx_ptr_tail->next = packet;
	  }
	  APP_tx_ptr_tail = packet;

	  packet = usb_malloc();
	  if(packet) {
		  Buf = packet->buf;
	  } else {
		  // usb_rx_memory_needed++; // TODO
	  }
  }

  packet->len = Len;
  packet->index = packet->buf[0];
*/
...
//--------------------------------------------------------------
uint16_t UB_VCP_StringRx(char *ptr)// TODO
{
  uint16_t akt_pos=0;
  //uint8_t wert;
  //uint32_t temptail;
